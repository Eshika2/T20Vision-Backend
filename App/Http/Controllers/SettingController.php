...
class SettingController extends Controller
{
    ...
    public function teamTeams (Request $request) {
        try {
            $data = json_decode($request->getContent(), true);
            $data['url'] = $request->url();
            $data['user_id'] = Auth::user()->id;

            $out_data = $this->settingRepository->teamTeams($data);
    ...
    public function teamVenues (Request $request) {
        try {
            $data = json_decode($request->getContent(), true);
            $data['url'] = $request->url();
            $data['user_id'] = Auth::user()->id;

            $out_data = $this->settingRepository->teamVenues($data);
